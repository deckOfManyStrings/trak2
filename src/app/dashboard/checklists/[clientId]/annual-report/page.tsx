...
import type { AnnualReport, Client } from "@/types/db";
...
export default async function AnnualReportListPage({ params }: PageProps) {
  ...
  const typedReports = (reports ?? []) as AnnualReport[];
    ...
    <div className="max-w-2xl space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-xl font-semibold">Annual Reports</h1>
        </div>
      ...
      {typedReports.length === 0 ? (
        <p className="rounded-lg border border-dashed bg-white p-6 text-center text-sm text-muted-foreground">
          No Annual Reports have been generated yet for {typedClient.full_name}.
        </p>
      ) : (
