...
import type { Client, QuarterlyReport } from "@/types/db";
...
export default async function QuarterlyReportListPage({ params }: PageProps) {
  ...
  const typedReports = (reports ?? []) as QuarterlyReport[];
    ...
    <div className="max-w-2xl space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-xl font-semibold">Quarterly Reports</h1>
        </div>

        <Link
          href={`/dashboard/checklists/${clientId}/quarterly-report/new`}
      ...
      {typedReports.length === 0 ? (
        <p className="rounded-lg border border-dashed bg-white p-6 text-center text-sm text-muted-foreground">
          No Quarterly Reports have been generated yet for{" "}
          {typedClient.full_name}.
        </p>
      ) : (
        <ul className="space-y-3">
          {typedReports.map((report) => (
            <li key={report.id}>
              <Link
                href={`/dashboard/checklists/${clientId}/quarterly-report/${report.id}`}
